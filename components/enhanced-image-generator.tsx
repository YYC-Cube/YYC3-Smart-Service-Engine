...
export function EnhancedImageGenerator({ onImageGenerated, onProgressUpdate }: EnhancedImageGeneratorProps) {
  ...
  const enhancePromptText = useCallback(
    (originalPrompt: string, selectedStyle: string): string => {
      ...
      const styleEnhancements = {
        ...
        "oil-painting": "oil painting, classical art, brush strokes, rich textures, artistic masterpiece",
        watercolor: "watercolor painting, soft colors, flowing paint, artistic, delicate brushwork",
        "digital-art": "digital art, concept art, modern illustration, clean lines, professional artwork",
  ...
  const simulateImageGeneration = useCallback(
    async (enhancedPrompt: string, currentSeed: number, abortSignal: AbortSignal): Promise<string> => {
      // 模拟生成进度
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < batchCount; i++) {
        const currentSeed = useRandomSeed ? generateRandomSeed() : Number.parseInt(seed) || generateRandomSeed()
        ...
        const imageUrl = await simulateImageGeneration(enhancedPrompt, currentSeed, abortControllerRef.current.signal)
          ...
          <Card className="bg-slate-800/50 border-slate-700/50">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Badge className="bg-green-500/20 text-green-300 border-green-500/50">YYC³ ImageGen v2.0</Badge>
              </CardTitle>
            </CardHeader>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Input
                      ...
                      max="4"
                      value={batchCount}
                      onChange={(e) => setBatchCount(Math.max(1, Math.min(4, Number.parseInt(e.target.value) || 1)))}
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Slider value={steps} onValueChange={setSteps} min={10} max={100} step={5} className="mt-3" />
                  <p className="text-xs text-slate-400 mt-2">更多步数 = 更高质量，但生成时间更长</p>
                </div>

                <div>
                  <div className="flex items-center space-x-2 mb-3">
                    <Switch id="random-seed" checked={useRandomSeed} onCheckedChange={setUseRandomSeed} />
